...
现在我们已经明白电脑是如何启动的，那也是时候创造我们自己的内核了。我们的小目标是，创建一个内核的磁盘映像，它能够在启动时，向屏幕输出一行“Hello World!”。我们的工作将基于上一章构建的独立式可执行程序。
...
哇哦，编译失败了！输出的错误告诉我们，Rust编译器现在不再能找到`core`或`compiler_builtins`库；而这两个库